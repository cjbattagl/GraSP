...
void gather_block_distribution_info(MPI_Comm comm, const int64_t local_perm_size, const int64_t global_perm_size, int64_t* perm_displs /* size = MPI comm size + 1 */, int** perm_owner_table_ptr /* malloc'ed in here */, int64_t** perm_owner_cutoff_ptr /* malloc'ed in here */, int* lg_minpermsize_ptr, int64_t* maxpermsize_ptr) {
  ...
  MPI_Allgather((void*)&local_perm_size, 1, MPI_INT64_T, perm_sizes, 1, MPI_INT64_T, comm);
...
void apply_permutation_mpi(MPI_Comm comm, const int64_t local_perm_size, const int64_t* const local_vertex_perm, const int64_t N, const int64_t nedges, packed_edge* result) {
  int rank, size;
...
#define LOOKUP_PERM_OWNER(v) \
  ...
  for (i = 0; i < nedges; ++i) {
    ...
    v0 = get_v0_from_edge(edge);
    v1 = get_v1_from_edge(edge);
    if (v0 >= 0) { owner_bitmap[LOOKUP_PERM_OWNER(v0)] = 1; }
    if (v1 >= 0) { owner_bitmap[LOOKUP_PERM_OWNER(v1)] = 1; }
  }
...
#define TEST_BIT(bm, idx) TEST_BIT_SCALAR((bm)[(idx) / ulong_bits], idx)
    ...
    /*for (i = 0; i < 2 * nedges; ++i) {
      ...
      int delta = (owner + size - rank) % size;
    ...
    for (i = 0; i < nedges; ++i) {
      ...
      if (v0 < 0) continue;
      int owner = LOOKUP_PERM_OWNER(v0);
      int64_t owner_displ = perm_displs[owner];
      int delta = (owner + size - rank) % size; /* Rank difference from mine, modulo size */
      if (delta >= offset && delta < offset + node_chunk_size) {
        unsigned long* bm = needed_element_bitmap + (delta - offset) * needed_element_bitmap_size;
        if (!TEST_BIT(bm, v0 - owner_displ)) {
          SET_BIT(bm, v0 - owner_displ);
          ++num_needed_elements[delta - offset];
        }
        SET_BIT(rescan_bitmap, i);
      }
    ...
    for (i = 0; i < nedges; ++i) {
      ...
      if (delta >= offset && delta < offset + node_chunk_size) {
        unsigned long* bm = needed_element_bitmap + (delta - offset) * needed_element_bitmap_size;
        if (!TEST_BIT(bm, v1 - owner_displ)) {
          SET_BIT(bm, v1 - owner_displ);
          ++num_needed_elements[delta - offset];
        }
        SET_BIT(rescan_bitmap, i);
      }
    ...
    for (delta = offset; delta < offset + num_nodes_being_processed; ++delta) {
      ...
      if (send_bitmap[src]) { /* Will I get any requests from src? */
        ...
        MPI_Irecv(incoming_perm_requests, maxpermsize, MPI_INT64_T, src, 0, comm, &incomingreq);
      }
      ...
      if (owner_bitmap[dest]) {
        ...
        MPI_Irecv(outgoing_perm_replies, output_offset, MPI_INT64_T, dest, 1, comm, &responsereq);
        /* Send the requests, using ssend for flow control (this can be changed
         * if your system has good flow control). */
        MPI_Ssend(outgoing_perm_requests, output_offset, MPI_INT64_T, dest, 0, comm);
      }
      if (send_bitmap[src]) {
        ...
        MPI_Rsend(incoming_perm_replies, count, MPI_INT64_T, src, 1, comm);
      }
      if (owner_bitmap[dest]) {
        ...
        for (block = 0; block < rescan_bitmap_size; ++block) {
          ...
          for (bit = 0; bit < ulong_bits; ++bit) {
            ...
            if (v >= 0 && LOOKUP_PERM_OWNER(v) == dest) {
              int64_t new_v = perm_replies_spread[v - perm_displ_for_dest] - N - 1;;
              if (i%2==0) { write_edge(&result[i/2], new_v, v1); }
              else { write_edge(&result[i/2], v0, new_v); }
            }
          }
        }
      }
...
#undef LOOKUP_PERM_OWNER
  ...
  for (i = 0; i < nedges; i++) {
    ...
    if (v0 != -1) {
      ...
      if (v1v < v2v) { write_edge(edge, v1v, v2v); }
      else { write_edge(edge, v2v, v1v); }
    }
  }
